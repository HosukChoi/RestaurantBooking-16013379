...
#include "../src/booking_scheduler.cpp"
...
TEST_F(BookingItem,
	시간대별인원제한이있다같은시간대에Capacity초과할경우예외발생) {
	Schedule* schedule = new Schedule{ ON_THE_HOUR ,CAPACITY_PER_HOUR, CUSTOMER };
	bookingScheduler.addSchedule(schedule);

	try {
		Schedule* new_schedule = new Schedule{ ON_THE_HOUR ,UNDER_CAPACITY, CUSTOMER };
...
TEST_F(BookingItem,
	시간대별인원제한이있다같은시간대가다르면Capacity차있어도스케쥴추가성공) {
	Schedule* schedule = new Schedule{ ON_THE_HOUR ,CAPACITY_PER_HOUR, CUSTOMER };
	bookingScheduler.addSchedule(schedule);

	tm different_hour = plusHour(ON_THE_HOUR, 1);

	Schedule* new_schedule = new Schedule{ different_hour ,UNDER_CAPACITY, CUSTOMER };
	bookingScheduler.addSchedule(new_schedule);

	EXPECT_EQ(true, bookingScheduler.hasSchedule(new_schedule));
}
...
TEST_F(BookingItem,
	...
	Schedule* schedule = new Schedule{
		ON_THE_HOUR, CAPACITY_PER_HOUR, CUSTOMER_WITH_MAIL };
...
TEST_F(BookingItem,
	...
	BookingScheduler* bookingScheduler =
		new TestableBookingScheduler(CAPACITY_PER_HOUR, getTime(2021, 3, 28, 17, 0));
...
TEST_F(BookingItem,
	현재날짜가일요일이아닌경우예약가능) {
	BookingScheduler* bookingScheduler =
		new TestableBookingScheduler(CAPACITY_PER_HOUR, getTime(2024, 6, 3, 17, 0));
